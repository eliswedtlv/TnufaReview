...
- **T-1141 — TNUFA-FAVICON-OG** (frontend + backend, `index.html` + `app.py` + assets) — done 2026-07-12.
  Gave the site a branded favicon and rich link preview. Created on-brand assets reusing index.html's palette (evergreen `--accent` #0f7a5a on warm paper): `favicon.svg` (rounded evergreen tile + white "T" monogram, legible at 16px), `favicon.ico` fallback, `apple-touch-icon.png` (180×180, full-bleed tile), and `og-image.png` (1200×630 card — "TnufaReview" wordmark + RTL Hebrew slogan "קופיילוט לבקשת מענק תנופה"). PNGs generated with Pillow (raqm RTL shaping). `<head>`: `rel="icon"` (svg + ico) + `apple-touch-icon` links and Open Graph/Twitter tags with **absolute** `og:image`/`twitter:image` = `https://tnufareview.com/og-image.png`. `app.py`: four `send_file` routes (`/favicon.ico`, `/favicon.svg`, `/apple-touch-icon.png`, `/og-image.png`) resolved against `app.root_path` — also fixes the `GET /favicon.ico → 404`. Out of scope and unchanged: `/review`, extraction, review prompt, response contract, model, copy. Tests: added a trivial `/favicon.ico → 200`; 9 passing.
- **T-1140 — TNUFA-REVIEW-OBSERVABILITY** (backend, `app.py` + `Procfile`) — done 2026-07-12.
  `/review` was slow then returned a 502/non-JSON gateway reply while the app logged nothing, so timeout-vs-error was undiagnosable. Added stdout logging via `logging.basicConfig` (INFO): request received, extraction start/done (elapsed seconds + extracted chars), review-call start/done (elapsed seconds), total elapsed; every `except` now logs `traceback.format_exc()` before returning the JSON error. Both OpenRouter calls (extraction + review) pass `timeout=240` so a hung upstream raises (already caught → clean JSON error) instead of hanging until gunicorn kills the worker. `Procfile`: worker `--timeout 300→600` (a legit slow review isn't killed) + `--access-logfile -/--error-logfile -` for gunicorn request logging on stdout. Out of scope and unchanged: review prompt, `instructions_form`, response contract, extraction logic, model. Tests: 8 passing.
